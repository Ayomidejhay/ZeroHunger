...
//   if (userType === "donor") return "/donordashboard";
//   if (userType === "recipient") return "/recipientdashboard";
//   return "/"; // fallback for unknown / admin roles
// }
...
//   const router = useRouter();
//     const searchParams = useSearchParams();
//   // If middleware set a ?next= param, honour it after login
//   const nextPath = searchParams.get("next");

//   // If the user is already authenticated (e.g. they navigate back to /login),
//   // send them to their dashboard once the profile has loaded.
//   useEffect(() => {
//     if (!authLoading && user && profile) {
//       router.replace(nextPath || dashboardFor(profile.user_type));
//     }
...
//       // userType is returned directly from signIn — no need to wait for
//       // the profile state to settle before routing.
//       // router.push(dashboardFor(userType));
//       router.push(nextPath || dashboardFor(userType));
//     } catch {
//       toast.error("Sign in failed", {
//         description: "Something went wrong. Please try again.",
//       });
