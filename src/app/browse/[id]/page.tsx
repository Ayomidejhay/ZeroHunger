...
export default function page() {
  ...
  const handleRequestFood = async () => {
    ...
    try {
      // Check if user already has an active reservation (pending OR confirmed) for
      // this listing. Checking only "pending" left a gap where a confirmed
      // reservation could be duplicated by re-requesting the same item.
      const { data: existingReservation, error: checkError } = await supabase
        .from("reservations")
        .select("id, status")
        .eq("food_listing_id", Array.isArray(id) ? id[0] : id ?? "")
      ...
      if (existingReservation) {
        const statusLabel = existingReservation.status === "confirmed"
          ? "confirmed"
          : "pending";
        toast.error("Already requested", {
          description: `You already have a ${statusLabel} request for this food item.`,
        });
        return;
      }
