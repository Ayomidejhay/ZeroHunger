...
interface ReservationTabsProps {
  ...
  onUpdate?: () => void; // refresh callback from the parent dashboard
}

const ReservationTabs = ({ reservations, stats, onUpdate }: ReservationTabsProps) => {
  const [activeTab, setActiveTab] = useState("pending");
  const [filters, setFilters] = useState({
    search: "",
    sortBy: "newest",
  ...
  const renderEmptyState = (status: string) => {
    ...
    return (
      <div className="text-center py-8">
        ...
        {hasActiveFilters && (
          <p className="text-sm text-neutral500">Try adjusting your search or filter criteria</p>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        {/* Pass onUpdate so PickupCard can refresh the dashboard after marking complete */}
        <TabsContent value="confirmed" className="mt-6">
          {confirmedReservations.length === 0
            ? renderEmptyState("confirmed")
            : <ConfirmedReservationsList reservations={confirmedReservations} onUpdate={onUpdate} />}
        </TabsContent>
